                        ...
                        {dentist.rating ? (
                          <>
                            <div className="flex items-center space-x-0.5">
                              {[1, 2, 3, 4, 5].map((star) => {
                                const filled = dentist.rating >= star
                                const half = !filled && dentist.rating >= star - 0.5
                                return (
                                  <svg key={star} className="w-4 h-4" viewBox="0 0 20 20">
                                    <defs>
                                      <linearGradient id={`half-${dentist.id}-${star}`}>
                                        <stop offset="50%" stopColor="#FBBF24" />
                                        <stop offset="50%" stopColor="#D1D5DB" />
                                      </linearGradient>
                                    </defs>
                                    <path
                                      fill={filled ? '#FBBF24' : half ? `url(#half-${dentist.id}-${star})` : '#D1D5DB'}
                            ...
                            <span className="ml-2 text-sm text-gray-600">
                              {dentist.rating} ({dentist.totalRatings} {dentist.totalRatings === 1 ? 'review' : 'reviews'})
                            </span>
                          </>
                        ) : (
